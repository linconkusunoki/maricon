      ...
      {confirmationList.map(
        (guest) =>
          guest && (
            <div
              key={guest.id}
              className="grid items-center w-full grid-cols-2 grid-rows-2 gap-4 p-4 border border-earth"
            >
              ...
              <select
                name={`${guest.id}-menu`}
                className="bg-[#D7D5B4] bg-opacity-50 text-bronze p-4 w-full placeholder-bronze outline-bronze uppercase"
              >
                <option>Menu</option>
